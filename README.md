...
- **反查脏码与废弃编码深度清洗**：全量清理原码表散落的 128 条无 `=` 裸单字母死条目（如旧版遗留的 `以\ty`、`一\ty`、`地\td`、`政\ti`、`主\tZ` 等）以及首选字重复候选衍生出的虚假补码（如 `以\ty.f=`），彻底解决拼音反查时一简字注释混入废弃一简或错误次选补码（例如「以」混入 `y` 和 `f=`）的顽疾，反查注释精确度达 100%。
...
空明码根据生理负荷与按键成本划分为 4 个层级。现代汉语 **Top 100 超高频汉字 100% 纳入一击体系**，所有一击字均为现代汉语排名前 204 名的核心汉字，日常单字一击直接上屏率达 **56.19%**。

> [!TIP]
> **一简字练习推荐**：
> 可以使用配套的终端打字练习工具 [dazitui](https://github.com/JackTheMico/dazitui) 来专门练习和巩固空明码一简字与空格并击字，快速建立肌肉记忆！
...
### 2. 26 字母八槽位一简字全景矩阵总表（全量 208 字）

收录空明码优化后的全部 **208 个一简字/一击字槽位**，涵盖物理单键、跨指小写、单手大写码元以及拇指空格并击。详细字频与指法对照参见 [`docs/kongmingma-yijian-table.md`](docs/kongmingma-yijian-table.md)：
...
- **Vim 风格全键盘极速操作**：
  ...
  - `x` 或 `<Delete>`：**条目删除**（底部常驻提示），唤起带同码词影响评估的确认卡片，底层基于墓碑标记（Tombstone）实现 O(1) 瞬时删除。
  ...
  - `d` 或 `Ctrl+r`：一键自动同步并触发 `fcitx5-rime` 重新部署（查看部署日志后按 Esc/Enter/q 可无缝返回编辑，不中断操作）。
  - `Ctrl+q`（或 `Ctrl+c`）：退出程序（未保存或部署时支持确认流程；普通模式下按 Esc 仅用于取消操作，避免误触退出）。
- **超大文件零卡顿**：支持 50MB+（逾 150 万行）词库秒级加载与毫秒级防抖持久化。
- **[`dazitui`](https://github.com/JackTheMico/dazitui)**：专为终端用户打造的打字练习 TUI 工具，支持专项练习空明码一简字与空格并击，快速肌肉记忆打卡。
